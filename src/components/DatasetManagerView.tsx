      ...
      <div className="rounded bg-[#161B22] border border-gray-800 p-4">
        ...
        <div className="overflow-x-auto max-h-96">
          <table className="w-full text-left text-xs text-gray-300 font-mono">
            <thead className="sticky top-0 bg-[#0B0E14] text-[10px] uppercase text-gray-500 border-b border-gray-800">
              <tr>
                <th className="p-2">Time</th>
                <th className="p-2">Event Name</th>
                <th className="p-2">Service</th>
                <th className="p-2">User Identity (ARN)</th>
            ...
            <tbody className="divide-y divide-gray-800/80">
              {filteredEvents.slice(0, 100).map((e) => (
                ...
                >
                  <td className="p-2 text-gray-500">{new Date(e.eventTime).toLocaleTimeString()}</td>
                  <td className="p-2 font-bold text-gray-200">
                    {e.eventName}
                    {e.isSimulatedAttack && (
                      <span className="ml-1.5 rounded bg-red-950/50 border border-red-800/60 px-1 py-0.2 text-[8px] font-bold text-red-400">
                        ATTACK
                      </span>
                    )}
                  </td>
                  <td className="p-2 text-blue-400">{e.eventSource}</td>
                  <td className="p-2 text-gray-400 truncate max-w-xs" title={e.userIdentity.arn}>
                    {e.userIdentity.arn || e.userIdentity.userName || 'Unknown'}
                  </td>
                  <td className="p-2 text-gray-500">{e.sourceIPAddress}</td>
                  <td className="p-2 text-right">
                    <button
                      onClick={() => onInspectRawEvent(e)}
                      className="rounded border border-gray-800 bg-[#0B0E14] px-1.5 py-0.5 text-[9px] font-medium text-gray-300 hover:bg-gray-800 hover:text-white"
                    >
                      Inspect JSON
                    </button>
                  </td>
